...
def terminar(signal_received, frame):
    print("\n[ENCERRAR] Ctrl+C detetado. A fechar recursos...")
    try:
        if camera_inicializada:
            camera.stop()
        pig.write(GPIO_BUZZER, 0)
        pig.write(GPIO_SINAL, 0)
        pig.stop()
        uart.close()
        print("[ENCERRAR] Recursos libertados com sucesso.")
    except Exception as e:
        print(f"[ERRO] Ao libertar recursos: {e}")
    sys.exit(0)
...
log_file_path = f"/mnt/fotos/mati1_log_{log_timestamp}.txt"
sys.stdout = open(log_file_path, "w", buffering=1)
print(f"[LOG] Log do programa guardado em: {log_file_path}")
...
def principal():
    ...
    try:
        ...
        print("[ENERGIA] Wi-Fi desativado para poupar energia.")
    except Exception as e:
        print(f"[ERRO] ao desativar Wi-Fi: {e}")
    ...
    with open(FICHEIRO_CSV, 'w', newline='') as f_csv:
        ...
        while True:
            ...
            print("[UART] Dados enviados com sucesso.")
...
if __name__ == "__main__":
    try:
        ...
        print("\n[SISTEMA] Encerrado pelo utilizador (Ctrl+C)")
        try:
            import pigpio
            pigpio.pi().stop()
        except Exception:
